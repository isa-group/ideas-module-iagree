...
public class AnalizeDelegate {
	...
	public static AppResponse analize(String id,
			Map<String, Object> wsagAggregation,
		...
		service.getService().addMetricFile(
				((String) wsagAggregation.get("metrics")).getBytes(),
				((String) wsagAggregation.get("metricUri")).getBytes());
		...
		if (id.equals("checkConsistency")) {
			try {
				...
				if (check) {
					appResponse
							.setMessage("<pre>The document is consistent.</pre>");
					appResponse.setStatus(Status.OK);
				} else {
					String inconsistencies = service
							.inconsitencyExplaining(document);
			...
			try {
				...
				if (numAlt == 0) {
					appResponse
							.setMessage("<pre>There are no alternatives for the document</pre>");
				} else {
					if (numAlt == 1) {
						appResponse.setMessage("<pre>There is " + numAlt
								+ " alternative to the document</pre>");
					} else {
						appResponse.setMessage("<pre>There are " + numAlt
								+ " alternatives to the document</pre>");
					}
				}
				appResponse.setStatus(Status.OK);
			} catch (Exception e) {
			...
			try {
				...
				service.getService()
						.addMetricFile(
								((String) wsagAggregationForComparation
										.get("metrics")).getBytes(),
								((String) wsagAggregationForComparation
										.get("metricUri")).getBytes());
				String otherDoc = (String) wsagAggregationForComparation
						.get("data");

				Boolean compliance = false;

				Boolean isCompliant = service.isCompliant(otherDoc, document);
				Boolean isLessRestrictiveOffer = service
						.isLessRestrictiveOffer(otherDoc, document);
				Boolean isMoreRestrictiveTemplateTermsThanCC = service
						.isMoreRestrictiveTemplateTermsThanCC(otherDoc);
				...
				if (compliance) {
					appResponse.setStatus(Status.OK);
					appResponse
							.setMessage("<pre>The provided offer is compliant with the provided template</pre>");
				} else {
					String explanation = service.nonComplianceExplaining(
							otherDoc, document);
					appResponse.setStatus(Status.OK_PROBLEMS);
					// appResponse.setMessage("<pre>" + explanation +
					// "</pre>");
